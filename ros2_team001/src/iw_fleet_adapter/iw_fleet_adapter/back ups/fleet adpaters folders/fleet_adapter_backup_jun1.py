    ...
    def follow_new_path(self, waypoints, arrival_cb, path_finished_cb):
        ...
        def monitor():
            ...
            for i, wp in enumerate(waypoints):
                ...
                # Retry up to 3 times on ABORTED (Nav2 rotation-only goal failure)
                MAX_RETRIES = 3
                for attempt in range(MAX_RETRIES):
                    ...
                    if self.api.navigation_succeeded(self.name):
...
# Fire arrival_cb only if robot reached the xy position
                data = self.api.get_data(self.name)
                if data:
                    dx = data.position[0] - pos[0]
                    dy = data.position[1] - pos[1]
                    at_waypoint = (dx*dx + dy*dy) < 0.5*0.5
                else:
                    at_waypoint = True  # fallback: assume reached
                
                if at_waypoint:
                    arrival_cb(i, delay)
                else:
                    self.node.get_logger().warn(
                        f"[{self.name}] Skipping arrival_cb at wp {i} — robot not at waypoint"
                    )
                prev_pos = pos
