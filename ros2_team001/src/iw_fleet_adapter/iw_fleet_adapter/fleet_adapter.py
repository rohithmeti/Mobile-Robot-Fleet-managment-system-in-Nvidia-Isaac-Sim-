...
class RobotAdapter:
    ...
    def stop(self, activity):
        ...
        # Only stop if the activity RMF wants to stop is the one we are actually executing!
        if self.execution is not None and self.execution.identifier.is_same(activity):
            ...
            self.node.get_logger().warn(f"[{self.name}] Ignored stale stop command. Sync protected.")          
        return True
    ...
    def update(self):
        ...
        # Only guard against genuine divergence — NOT cov==0.0.
        # In this deterministic sim, a stationary/converged robot can
        # legitimately report exactly 0.0 covariance.
        ...
        # Push the current physical state up to the RMF traffic scheduler.
        # update()'s 2nd arg must be an ActivityIdentifier (or None when
        # idle) — NOT a bool. This is what the "incompatible function
        # arguments" error was telling you: is_command_completed(name)
        # returns a bool, but update() expects self.execution.identifier.
        state = rmf_easy.RobotState(data.map, data.position, 1.0)
        if self.update_handle is not None:
            current_activity = (
                self.execution.identifier if self.execution is not None else None
            )
            self.update_handle.update(state, current_activity)
...
# ==============================================================================
    ...
    # Use the exact attribute names found in the 2.12 API
    fleet_config.default_max_merge_waypoint_distance = 0.5   
    fleet_config.default_max_merge_lane_distance = 1.0
    
    fleet_handle = adapter.add_easy_fleet(fleet_config)
    
    # RESTORED MISSING DICTIONARY AND FOR LOOP
    robot_adapters = {}
    for robot_name in config_yaml['rmf_fleet']['robots']:
        
        # FIXED: Use 'robot_name' instead of 'iw_hub_fleet'
        ...
        while data is None:                          
            # FIXED: Use 'robot_name' instead of 'iw_fleet_hub'
